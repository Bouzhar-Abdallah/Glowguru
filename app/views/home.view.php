<!DOCTYPE html>
<html class="scroll-smooth" lang="en">

<head>
  <meta charset="UTF-8" />
  <meta http-equiv="X-UA-Compatible" content="IE=edge" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
  <!-- <link
      href="https://fonts.googleapis.com/css2?family=Raleway:ital,wght@0,100;0,200;0,300;0,400;0,500;0,600;1,100&display=swap"
      rel="stylesheet"
    /> -->
  <script src="https://cdn.tailwindcss.com"></script>
  <!-- <link rel="stylesheet" href="./src/style.css"> -->

  <style>
    @import url('https://fonts.googleapis.com/css2?family=Raleway:ital,wght@0,100;0,200;0,300;0,400;0,500;0,600;1,100&family=Secular+One&display=swap');

    .font-Raleway {
      font-family: Raleway;
    }

    .font-secular {
      font-family: 'Secular One';
    }
  </style>
  <title>Document</title>
</head>

<body class="font-Raleway ">

  <nav id="nav" class="flex items-center z-50 p-2 justify-between bg-[#8cd0e3] text-white top-0 ">
    <div class="w-10 h-10 grid place-items-center">
      <!-- <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
        class="w-6 h-6">
        <path stroke-linecap="round" stroke-linejoin="round"
          d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
      </svg> -->
    </div>


    <div class="mx-auto">
      <div class="flex flex-col justify-center items-center">
        <h1 class="text-4xl font-medium relative font-secular my-1">GlowGuru <span
            class="text-sm absolute top-1">&copy</span></h1>
      </div>
      <hr class="shadow-xl">

      <ul class="flex justify-between font-medium text-xl">
        <li class="p-2">cat1</li>
        <li class="p-2">cat1</li>
        <li class="p-2">cat1</li>

      </ul>
    </div>
    <div class="w-10 h-10 grid place-items-center">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
        class="w-6 h-6">
        <path stroke-linecap="round" stroke-linejoin="round"
          d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" />
      </svg>
    </div>
  </nav>
  <!-- slider start -->
  <div class="bg-[#f3f8fe] pb-8 my-8">
    <div class="grid place-items-center my-8 pt-10">
      <h1 class=" text-3xl font-bold">New Arrivals</h1>
      <h1 class="text-center text-lg font-light px-4">Our Cold Weather Staples Will Help Keep Skin Hydrated & Happy All
        Winter Long</h1>
    </div>
    <section class=" flex items-center justify-around lg:mx-12">
      <button id="left" class=" mx-4 bg-white hover:bg-slate-100\ rounded-full w-8 h-8 grid place-items-center">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
          class="w-6 h-6">
          <path class="w-5 h-5" stroke-linecap="round" stroke-linejoin="round"
            d="M18.75 19.5l-7.5-7.5 7.5-7.5m-6 15L5.25 12l7.5-7.5" />

        </svg>
      </button>

      <div id="carousel" class="relative overflow-hidden w-full">

        <div id="slider" class="flex absolute transition-all ease-in-out duration-200 cursor-pointer">

          <div class="mx-0.5 relative bg-white">
            <div class="test relative">
              <img class="absolute " src="./src/images/first.webp" alt="" srcset="">
              <img class="seocnd_image absolute z-10 hidden" src="./src/images/second.webp" alt="" srcset="">
            </div>

            <div class="flex flex-col items-center capitalize my-2 gap-0.5 relative py-2">
              <h1 class="font-bold text-xl text-center relative text-gray-900">libelle</h1>
              <h1 class="font-normal text-sm text-center text-gray-600">descritpion</h1>
              <div class="hidden">
                39.39 MAD
              </div>
            </div>

          </div>

          <div class=" mx-0.5 relative bg-white">
            <div class="test relative">
              <img class="absolute " src="./src/images/2-first.webp" alt="" srcset="">
              <img class="seocnd_image absolute z-10 hidden" src="./src/images/2-second.webp" alt="" srcset="">
            </div>
            <div class="absolute bottom-0 w-full">
              <div class="flex flex-col items-center capitalize my-2 gap-0.5 relative">
                <h1 class="font-bold text-xl text-center relative text-gray-900">libelle</h1>
                <h1 class="font-normal text-sm text-center text-gray-600">descritpion</h1>
                <div class="hidden">
                  39.39 MAD
                </div>
              </div>
            </div>
          </div>

          <div class=" mx-0.5 relative bg-white">
            <div class="test relative">
              <img class="absolute " src="./src/images/3-first.webp" alt="" srcset="">
              <img class="seocnd_image absolute z-10 hidden" src="./src/images/3-second.webp" alt="" srcset="">
            </div>
            <div class="absolute bottom-0 w-full">
              <div class="flex flex-col items-center capitalize my-2 gap-0.5 relative">
                <h1 class="font-bold text-xl text-center relative text-gray-900">libelle</h1>
                <h1 class="font-normal text-sm text-center text-gray-600">descritpion</h1>
                <div class="hidden">
                  39.39 MAD
                </div>
              </div>
            </div>
          </div>

          <div class=" mx-0.5 relative bg-white">
            <div class="test relative">
              <img class="absolute " src="./src/images/4-first.webp" alt="" srcset="">
              <img class="seocnd_image absolute z-10 hidden" src="./src/images/4-second.jpeg" alt="" srcset="">
            </div>
            <div class="absolute bottom-0 w-full">
              <div class="flex flex-col items-center capitalize my-2 gap-0.5 relative">
                <h1 class="font-bold text-xl text-center relative text-gray-900">libelle</h1>
                <h1 class="font-normal text-sm text-center text-gray-600">descritpion</h1>
                <div class="hidden">
                  39.39 MAD
                </div>
              </div>
            </div>
          </div>

          <div class=" mx-0.5 relative bg-white">
            <div class="test relative">
              <img class="absolute " src="./src/images/1-hair-first.webp" alt="" srcset="">
              <img class="seocnd_image absolute z-10 hidden" src="./src/images/1-hair-second.webp" alt="" srcset="">
            </div>
            <div class="absolute bottom-0 w-full">
              <div class="flex flex-col items-center capitalize my-2 gap-0.5 relative">
                <h1 class="font-bold text-xl text-center relative text-gray-900">libelle</h1>
                <h1 class="font-normal text-sm text-center text-gray-600">descritpion</h1>
                <div class="hidden">
                  39.39 MAD
                </div>
              </div>
            </div>
          </div>

          <div class=" mx-0.5 relative bg-white">
            <div class="test relative">
              <img class="absolute " src="./src/images/2-hair-first.webp" alt="" srcset="">
              <img class="seocnd_image absolute z-10 hidden" src="./src/images/2-hair-second.webp" alt="" srcset="">
            </div>
            <div class="absolute bottom-0 w-full">
              <div class="flex flex-col items-center capitalize my-2 gap-0.5 relative">
                <h1 class="font-bold text-xl text-center relative text-gray-900">libelle</h1>
                <h1 class="font-normal text-sm text-center text-gray-600">descritpion</h1>
                <div class="hidden">
                  39.39 MAD
                </div>
              </div>
            </div>
          </div>

          
           

          
            
          </div>



        </div>

      
      <button id="right" class=" mx-4 bg-white hover:bg-slate-100\ rounded-full w-8 h-8 grid place-items-center">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
          class="w-6 h-6">
          <path class="w-5 h-5" stroke-linecap="round" stroke-linejoin="round"
            d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" />
        </svg>
      </button>
    </section>
  </div>
  <!-- end slider -->

<!-- grid start -->
  <div class="m-10">
    <div class="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-5">

      <div class="bg-white lg:mx-4 overflow-hidden">
        <div class="relative">
          <div class="absolute opacity-0 hover:opacity-100 hover:bg-black/10 w-full h-full grid place-items-center transition-all duration-300">
            <a class="border px-4 py-2 bg-white/30 border-black hover:bg-white transition-all duration-500 cursor-pointer font-medium">Check it out</a>
          </div>
          <img class="w-full" src="<?=ROOT?>public/src/images/KJC_VLK_22_Lip Macros_Charm_01.webp" alt="Card Image">
        </div>
        <div class="p-4">
          <h2 class="text-lg font-medium text-gray-900">Card Title</h2>
          <p class="text-gray-600">Card Description</p>
        </div>
      </div>

      <div class="bg-white lg:mx-4 overflow-hidden">
        <img class="w-full" src="./src/images/KJC_VLK_22_Lip Macros_Charm_01.webp" alt="Card Image">
        <div class="p-4">
          <h2 class="text-lg font-medium text-gray-900">Card Title</h2>
          <p class="text-gray-600">Card Description</p>
        </div>
      </div>
      <div class="bg-white lg:mx-4 overflow-hidden">
        <img class="w-full" src="./src/images/KJC_VLK_22_Lip Macros_Charm_01.webp" alt="Card Image">
        <div class="p-4">
          <h2 class="text-lg font-medium text-gray-900">Card Title</h2>
          <p class="text-gray-600">Card Description</p>
        </div>
      </div>
      <div class="bg-white lg:mx-4 overflow-hidden">
        <img class="w-full" src="./src/images/KJC_VLK_22_Lip Macros_Charm_01.webp" alt="Card Image">
        <div class="p-4">
          <h2 class="text-lg font-medium text-gray-900">Card Title</h2>
          <p class="text-gray-600">Card Description</p>
        </div>
      </div>
      <div class="bg-white lg:mx-4 overflow-hidden">
        <img class="w-full" src="./src/images/KJC_VLK_22_Lip Macros_Charm_01.webp" alt="Card Image">
        <div class="p-4">
          <h2 class="text-lg font-medium text-gray-900">Card Title</h2>
          <p class="text-gray-600">Card Description</p>
        </div>
      </div>
      <div class="bg-white lg:mx-4 overflow-hidden">
        <img class="w-full" src="./src/images/KJC_VLK_22_Lip Macros_Charm_01.webp" alt="Card Image">
        <div class="p-4">
          <h2 class="text-lg font-medium text-gray-900">Card Title</h2>
          <p class="text-gray-600">Card Description</p>
        </div>
      </div>
      <div class="bg-white lg:mx-4 overflow-hidden">
        <img class="w-full" src="./src/images/KJC_VLK_22_Lip Macros_Charm_01.webp" alt="Card Image">
        <div class="p-4">
          <h2 class="text-lg font-medium text-gray-900">Card Title</h2>
          <p class="text-gray-600">Card Description</p>
        </div>
      </div>
      <div class="bg-white lg:mx-4 overflow-hidden">
        <img class="w-full" src="./src/images/KJC_VLK_22_Lip Macros_Charm_01.webp" alt="Card Image">
        <div class="p-4">
          <h2 class="text-lg font-medium text-gray-900">Card Title</h2>
          <p class="text-gray-600">Card Description</p>
        </div>
      </div>
      <div class="bg-white lg:mx-4 overflow-hidden">
        <img class="w-full" src="./src/images/KJC_VLK_22_Lip Macros_Charm_01.webp" alt="Card Image">
        <div class="p-4">
          <h2 class="text-lg font-medium text-gray-900">Card Title</h2>
          <p class="text-gray-600">Card Description</p>
        </div>
      </div>
    
      
    </div>
  </div>
<!-- grid end -->







</body>

</html>
<script src="./src/script.js"></script>